...
class _StepperState extends State<CustomStepper> with TickerProviderStateMixin {
  ...
  ///图标   
  Widget _iconStyle(int index) {
    if(widget.currentStep==0){
      return Icon(Icons.radio_button_checked,color: Colors.green,);
    }
    return Icon(widget.steps[index].isActive && index < widget.currentStep
          ? Icons.check_circle
  ...
  Widget _buildHorizontal() {
    ...
    for (int i = 0; i < widget.steps.length; i += 1) {
      ...
      if(i == 0){
        child = Container(
          margin: EdgeInsets.only(left: MediaQuery.of(context).size.width / widget.steps.length / 2 - _kStepSize -10 ),
          child: _buildIcon(i),
        );
      } else if (_isLast(i)){
        child = Container(
          margin: EdgeInsets.only(right: MediaQuery.of(context).size.width / widget.steps.length / 2 - _kStepSize -10),
          child: _buildIcon(i),
        );
      }else{
        child = _buildIcon(i);
